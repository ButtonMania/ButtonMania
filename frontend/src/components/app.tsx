...
export default class App extends Component<Props, State> {
  ...
  constructor(public props: Props) {
    ...
    MessageBus.default(UpdateResponse).subscribe(
      this.onRecieveUpdate.bind(this),
    );
    MessageBus.default(RecordResponse).subscribe(
      this.onRecieveRecord.bind(this),
    );
    ...
    MessageBus.default(DisconnectEvent).subscribe(
      this.onStopGameplay.bind(this),
    );
  }
  ...
  componentWillMount(): void {
    ...
    var maxType: number = enumIndex(
      ButtonType,
      isPremium ? ButtonType.Prestige : ButtonType.Fortune,
    );
    var minType: number = enumIndex(
      ButtonType,
      isNewYear ? ButtonType.NewYear : ButtonType.Peace,
    );
    let buttonType: ButtonType = isNewYear
      ? ButtonType.NewYear
      : randomEnum(ButtonType, minType, maxType);
  ...
  onChangeButtonPhase(buttonPhase: ButtonPhase, holdDuration: number): void {
    ...
    switch (buttonPhase) {
      ...
      case ButtonPhase.Push: {
        ...
        this.client.startGameplay(this.state.initData, this.state.buttonType);
        break;
      }
      case ButtonPhase.Hold: {
      ...
      case ButtonPhase.Release: {
        MessageBus.raise(new GameplayMessageRequest(buttonPhase)).catch(
          console.log,
        );
        break;
      }
  ...
  onRecieveError(respons: ErrorResponse): void {
    this.props.webApp.showPopup({
      message: `Oops! An error has occurred: ${respons.error.message}`,
    });
  ...
  render(props: Props, state: State): ComponentChild {
    return (
      // @ts-expect-error preact/compat and JSX factory type issue workaround
